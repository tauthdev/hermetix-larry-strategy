...
/**
 * Larry Williams 식 변동성 돌파 전략 (롱 온리).
 *
 ...
 *      이상인 양봉이면, 주문 가능 현금의 budgetRatio 만큼 시장가 매수.
...
class LarryStrategy(
    ...
    /** 마지막으로 판정한 캔들 시각 — 같은 캔들로 중복 진입하지 않기 위함 */
    @Volatile
    internal var lastEvaluated: Instant? = null

    /** 진입 시각 — 만료 청산 판정용. 재시작 시 보유가 발견되면 그 시점부터 다시 센다 */
    @Volatile
    ...
    private fun decideEntry(symbol: String, context: StrategyContext): List<Signal> {
        ...
        if (target.open >= target.close) {
            logger.info { "하락 돌파 감지 - 롱 온리 전략이므로 스킵 / body=$targetBody avg=$avgBody" }
            return emptyList()
        }
        ...
        logger.info { "진입 / $symbol qty=$quantity price=$price 손절가=${target.open} (body=$targetBody >= $threshold)" }
